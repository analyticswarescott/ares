...
public class KafkaTopicPartitionStatusTest {
	...
	public void test_toFromJson() throws Exception {
		...
		KafkaTopicPartitionStatus status = new KafkaTopicPartitionStatus(Tenant.forId("1"), Topic.EVENTS_ES, 1, new TopicMessage(Instant.ofEpochMilli(1000), 100), new TopicMessage(Instant.ofEpochMilli(2000), 200));
